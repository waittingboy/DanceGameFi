...
contract MysteryBoxMock is Modifiers, IMysteryBox {
    ...
    // the following functions are added for test
    function getIsAdmin(address _user) public view returns (bool) {
        return isAdmin[_user];
    }

    function getDataRang(uint _propId) public view returns (uint, uint, uint) {
        NFTData storage data = boxPool.NFTDatas[_propId];
        uint rangLength = data.rang.length;
        TokenIdRang memory lastRang;
        if (rangLength == 0) {
            return (0, 0, 0);
        }
        lastRang = data.rang[rangLength - 1];
        return (rangLength, lastRang.start, lastRang.end);
    }

    function setDataRemainZero(uint _propId) public {
        NFTData storage data = boxPool.NFTDatas[_propId];
        data.remainedQuantity = 0;
    }
}
